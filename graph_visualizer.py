...
def plot_graphs_4(G,N,legend,file_name, names):
    plt.axis('off')
    ...
    ax2.title.set_text(names[1])
    ax3.title.set_text(names[2])
    ax4.title.set_text(names[2])
       #print("computing layout")
    pos =graphviz_layout(G[0], prog='dot')

    nx.draw_networkx_nodes(G[0],pos,node_color=N[0],ax=ax1)
    nx.draw_networkx_edges(G[0],pos,ax=ax1)
    nx.draw_networkx_labels(G[0],pos,ax=ax1)

    pos2 =graphviz_layout(G[1], prog='dot')

    nx.draw_networkx_nodes(G[1],pos2,node_color=N[1],ax=ax2)
    nx.draw_networkx_edges(G[1],pos2,ax=ax2)
    nx.draw_networkx_labels(G[1],pos2,ax=ax2)

    pos3 =graphviz_layout(G[2], prog='dot')

    nx.draw_networkx_nodes(G[2],pos3,node_color=N[2],ax=ax3)
    nx.draw_networkx_edges(G[2],pos3,ax=ax3)
    nx.draw_networkx_labels(G[2],pos3,ax=ax3)

    pos4 =graphviz_layout(G[3], prog='dot')

    nx.draw_networkx_nodes(G[3],pos4,node_color=N[3],ax=ax4)
    nx.draw_networkx_edges(G[3],pos4,ax=ax4)
    nx.draw_networkx_labels(G[3],pos4,ax=ax4)
...
def plot_generated_comparison(data,data2,loc="."):
    for i in range(min(len(data["gen"]),len(data2["gen"]))):
        if np.sum(data["A"][i] - data2["A"][i]) != 0:
            ...
            for j in range(len(x_gen)):
                ...
                if n == 0:
                    ...
                    legend[n] = mpatches.Patch(color=cmap(n), label=classes_rev[n])
                    colors_pred.append(cmap(n))
            plot_graphs_comparison(G_orig, colors_orig, G_pred, colors_pred, G_pred2, colors_pred,legend.values(),f"./predictions/generated_comparison2_3_15/{i}.png",names=["Original", "Model2", "Model3"])
...
def check_connectivity(data):
    ...
    print(f"Unconnected ratio={unconnected/total_added}")
    print(f"Average amount of connections for user nodes: Orginial:{original_user_edges/original_users}, Generated:{generated_user_edges/(total_added - unconnected)}, G/O:{(generated_user_edges/total_added)/(original_user_edges/original_users)}")
...
def validity_ratio(data):
    ...
    for i in range(len(data["gen"])):
        ...
        #count connections
        connections = np.sum(a_gen[len(x):,len(x):],axis=1)
        #take only the new nodes
            ...
            for edge in G.in_edges(n):
                ...
                if x[parent] == 2:
                    if not in_ou:
                        valid_edges += 1
                        in_ou = True
                elif x[parent] == 3 or x[parent] == 4:
                    valid_edges += 1
    print(f"Ratio of valid edges:{valid_edges/predicted_edges}")
...
if __name__ == '__main__':
    ...
    with open(sys.argv[2], "rb") as infile2:
        data2 = pickle.load(infile2)
    #check_connectivity(data)
    #validity_ratio(data)
    print("----------------------------------")
    #plot_reconstructed_graphs(data,threshold=0.35)

    #plot_generated_graphs(data2)
    plot_generated_comparison(data,data2)
    """
    for i in range(len(data["gen"])):
        x = data["X"][i]
        a = data["A"][i]
        ...
        gen = data["gen"][i]
        a_hat = np.where(a_hat > threshold,1,0)
        #gen = np.where(gen > threshold,1,0)
        
        x_new = np.concatenate([x,6*np.ones(gen.shape[0])])
        a_new = np.vstack([a,gen])
        a_new = np.hstack([a_new, np.zeros([a_new.shape[0],1])])
        a_new = np.where(a_new > threshold, 1,0)

        G_orig = nx.from_numpy_array(np.transpose(a), create_using=nx.DiGraph)
        G_pred = nx.from_numpy_array(np.transpose(a_hat), create_using=nx.DiGraph)
        G_gen = nx.from_numpy_array(np.transpose(a_new), create_using=nx.DiGraph)
        ...
        for j in range(len(x_new)):
            ...
            if n == 0:
                ...
                generated = True
                legend[n] = mpatches.Patch(color=cmap(0), label=classes_rev[n])
                colors_gen.append(cmap(0))
            else:
                legend[n] = mpatches.Patch(color=cmap(n), label=classes_rev[n])
                colors_gen.append(cmap(n))

        if generated:
            plot_graphs(G_orig,colors_orig, G_gen,colors_gen,legend.values(),f"./predictions/Type3/graph_{i}_generated.png",["original","generated"])
        legend.pop(6)
        plot_graphs(G_orig,colors_orig, G_pred,colors_pred,legend.values(),f"./predictions/Type3/graph_{i}.png",["original","reconstructed"])

    """
    
    #files = ["./Experiments/Type1/m=1_rdim64_50_comparison.pickle","./Experiments/Type2/m=2_rdim64_50_comparison.pickle","./Experiments/Type3/m=3_rdim64_50_comparison.pickle"]
    #files = ["./Experiments/Type3/m=3_rdim64_15_comparison.pickle","./Experiments/Type3/m=3_rdim64_50_comparison.pickle","./Experiments/Type3/m=3_rdim64_150_comparison.pickle","./Experiments/Type3/m=3_rdim64_500_comparison.pickle"]

    #plot_roc_curve(files, "roc_model1_all_datasets.png")
    #plot_pr_curve(files, "pr_model3_all_datasets.png")

    #plot_Acomparison_rgb(files,0.15, "./Experiments/comp2_3_500")